...
def main():
	...
	if mode == "pre":
		...
		if rat_mode == "mc":
			...
			lista = get_neighbor_shells(sc,6)
	...
	if mode == "scph":
		...
		os.makedirs('FORCE_CONSTANTS/', exist_ok=True)
		...
		for i, T in enumerate(temp):
			...
			if i > 0:
				...
				if os.path.isdir('scph_trajs'):
					...
					if files:
						...
						fcp = ForceConstantPotential.read('Potentials/Potential_T{}.0.fcp'.format(highest_temperature))
						print("Using parameters from temperature:", highest_temperature, "K")
			...
			fcp_scph.write('Potentials/Potential_T{}.fcp'.format(T))
			...
			#Print ShengBTE files
			print("Printing files")
			fcs_scph.write_to_phonopy("FORCE_CONSTANTS/FORCE_CONSTANTS_2ND_scph_{}".format(T), format="text")
